...
namespace Comet
{
	...
	{
		viewport = *(renderer->GetViewports().begin());
		...
		cam->SetViewport(*(renderer->GetViewports().begin()));
	...
	bool BaseGame::Update()
	{
		...
		{
			viewport->GetCamera()->GetNode()->Translate(glm::vec3(0.0f, 0.0f, -5.03f*dt), 0);
		}
		...
		{
			viewport->GetCamera()->GetNode()->Translate(glm::vec3(-5.03f*dt, 0.0f, 0.0f), 0);
		}
